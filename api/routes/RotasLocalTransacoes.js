...
class rotasLocalTransacoes {
    ...
    static async novaLocalTransacoes(req, res) {
        const { id_usuario, id_transacao } = req.body;
        try {
            const localTransacao = await BD.query(`INSERT INTO local_transacoes (id_usuario, id_transacao)
                    VALUES ($1, $2)`,
                [id_usuario, id_transacao]);                
            res.status(201).json(localTransacao); //retorna o usuario criado com status 201
        } catch (error) {
    ...
    static async listarTransacoes(req, res) {
        try {
            const transacoes = await BD.query('SELECT * FROM local_transacoes');
    ...
    static async listarTransacoesPorID(req, res) {
        ...
        try {   
            const transacao = await BD.query('SELECT * FROM local_transacoes WHERE id_transacao = $1', [id_transacao]);
    ...
    static async atualizarTransacoes(req, res) {
        ...
        try {
            const transacao = await BD.query(`UPDATE local_transacoes SET id_usuario = $1 WHERE id_transacao = $2 RETURNING *`,
                [id_usuario, id_transacao]);
